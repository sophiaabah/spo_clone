...
export function draw(img) {
  ...
  let [dominantColorFreq] = Object.values(color).sort((a, b) => b - a);
  console.log("dominant", dominantColorFreq);

  let dominantColorArr = colorFrequencyPairs?.find(([color, freq]) => {
    return freq === dominantColorFreq;
  });

  console.log("result", dominantColorArr);
  return `#${dominantColorArr?.[0].toString().padEnd(6, "0")}`;
}
...
function rgbToHex(r, g, b, a) {
  ...
  //why are we converting rgb to hex? can i do without it?
}
